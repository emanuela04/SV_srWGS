...
def load_coverage_to_pyranges(input_file):
    # Nome del sample preso direttamente dal VCF sarà usato come key
    ...
    # Usa "sample" come chiave temporanea, poi sostituire con SAMPLE_NAME del VCF
...
def combine_bed_files_single_tsv(gc_pr, coverage_dict, sv_df, output_file):
    ...
    header = [
        ...
        "SVLEN_CALLER", "MANTA", "DELLY", "SMOOVE", "OVERLAPS_REPEATS",
        "OVERLAPS_SEG_DUP", "GC_CONTENT", "COVERAGE_MOSDEPTH"
    ]
        ...
        for sample_name in unique_samples:
            ...
            for _, row in sample_sv_df.iterrows():
                ...
                row_data = [
                    ...
                    row.get("MANTA", 0), row.get("DELLY", 0), row.get("SMOOVE", 0),
...
def main():
    ...
    parser.add_argument("--repeat_masker", required=True, help="File with RepeatMasker TSV file")
    parser.add_argument("--segmental_duplications", required=True, help="File with Segmental Duplications TSV file")
    parser.add_argument("--vcf_file", required=True, help="VCF file")
    parser.add_argument("--cg_content", required=True, help=" CG content file (_merged_gc_content.txt)")
    parser.add_argument("--coverage_mosdepth", required=True, help=" Mosdepth coverage file (.regions.bed.gz)")
